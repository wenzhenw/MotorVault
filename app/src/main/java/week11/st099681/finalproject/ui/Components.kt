...
val bottomTabs = listOf(
    ...
    BottomTab("fuel", "Fuel", Icons.Filled.LocalGasStation),
...
/** Reusable Yes/No confirmation dialog — used for every delete action (CRUD: Delete). */
@Composable
fun ConfirmDialog(
    title: String,
    message: String,
    ...
    AlertDialog(
        ...
        containerColor = week11.st099681.finalproject.ui.theme.NavyCard
    )
}

/** Small icon-only round button, used for edit/delete affordances on list rows. */
@Composable
fun RowIconButton(icon: ImageVector, tint: Color = TextSecondary, contentDescription: String? = null, onClick: () -> Unit) {
    Box(
        modifier = Modifier
            .size(32.dp)
            .background(Slate, CircleShape)
            .clickable { onClick() },
        contentAlignment = Alignment.Center
    ) {
