...
fun DashboardScreen(
    ...
    val fuelLogs = vm.fuelLogsForSelected()
    val fuelSpend = fuelLogs.sumOf { it.totalCost ?: 0.0 }
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        if (vehicle != null) {
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                PrimaryButton("Upload Receipt", modifier = Modifier.weight(1f)) { onUploadReceipt() }
                SecondaryButton("Log Fuel", modifier = Modifier.weight(1f)) { onLogFuel() }
            }
            Spacer(Modifier.height(14.dp))

            // ---- fuel quick-stat card ----
            Card(
                colors = CardDefaults.cardColors(containerColor = NavyCard),
                shape = RoundedCornerShape(12.dp),
                modifier = Modifier
                    .fillMaxWidth()
                    .clickable { onViewFuelHistory() }
            ) {
                Row(
                    verticalAlignment = Alignment.CenterVertically,
                    ...
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Fuel Spend", fontWeight = FontWeight.SemiBold, fontSize = 14.sp)
                        Text(
                            "${fuelLogs.size} fill-up${if (fuelLogs.size == 1) "" else "s"} logged",
                            color = TextSecondary,
                            fontSize = 11.sp
                        )
                    }
                    Text("$" + "%.2f".format(fuelSpend), color = Green, fontWeight = FontWeight.Bold, fontSize = 15.sp)
                }
            }
