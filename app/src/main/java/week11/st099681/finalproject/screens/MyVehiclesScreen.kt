...
fun MyVehiclesScreen(
    ...
    vm.services.collectAsState() // subscribe so overall status recomposes with new data
    ...
    pendingDelete?.let { vehicle ->
        ConfirmDialog(
            ...
            message = "This removes the vehicle profile. Its service history and fuel logs will stay on record but won't be linked to a vehicle anymore.",
            onConfirm = {
                vm.deleteVehicle(vehicle.id) { ok ->
                    if (!ok) Toast.makeText(context, "Could not delete vehicle", Toast.LENGTH_SHORT).show()
                }
                pendingDelete = null
            },
        ...
        Column(modifier = Modifier.padding(14.dp)) {
            Box(modifier = Modifier.fillMaxWidth()) {
                ...
                Row(
                    ...
                    RowIconButton(Icons.Filled.Edit, contentDescription = "Edit vehicle", onClick = onEdit)
                    RowIconButton(Icons.Filled.Delete, tint = Red, contentDescription = "Delete vehicle", onClick = onDelete)
                }
            }
