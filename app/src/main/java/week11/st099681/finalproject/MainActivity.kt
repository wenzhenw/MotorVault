...
import week11.st099681.finalproject.screens.AddFuelScreen
...
import week11.st099681.finalproject.screens.FuelHistoryScreen
...
object Routes {
    ...
    const val EDIT_VEHICLE = "add_vehicle/{vehicleId}"
    fun editVehicle(vehicleId: String) = "add_vehicle/$vehicleId"
    ...
    const val FUEL_HISTORY = "fuel"
    const val ADD_FUEL = "add_fuel"
    const val EDIT_FUEL = "add_fuel/{fuelId}"
    fun editFuel(fuelId: String) = "add_fuel/$fuelId"
}
    ...
    NavHost(navController = navController, startDestination = Routes.SPLASH, modifier = modifier) {
        ...
        composable(Routes.VEHICLES) {
            MyVehiclesScreen(
                ...
                onEditVehicle = { vehicle -> navController.navigate(Routes.editVehicle(vehicle.id)) },
        ...
        composable(Routes.DASHBOARD) {
            DashboardScreen(
                ...
                onLogFuel = { navController.navigate(Routes.ADD_FUEL) },
                onViewFuelHistory = {
                    navController.navigate(Routes.FUEL_HISTORY) {
        ...
        composable(Routes.RESULTS) {
            OcrResultsScreen(
                ...
                onBack = { navController.popBackStack() },
                onSaved = { type ->
                    val target = if (type == ReceiptType.FUEL) Routes.FUEL_HISTORY else Routes.HISTORY
        ...
        composable(Routes.FUEL_HISTORY) {
            FuelHistoryScreen(
                vm = vm,
                onAddFuel = { navController.navigate(Routes.ADD_FUEL) },
                onEditFuel = { log -> navController.navigate(Routes.editFuel(log.id)) }
            )
        }
        ...
        composable(
            route = Routes.EDIT_FUEL,
            arguments = listOf(navArgument("fuelId") { type = NavType.StringType })
        ) { entry ->
            val fuelLogs by vm.fuelLogs.collectAsState()
            val fuelId = entry.arguments?.getString("fuelId")
            val log = fuelLogs.firstOrNull { it.id == fuelId }
            AddFuelScreen(
                vm = vm,
                editFuel = log,
                onBack = { navController.popBackStack() },
